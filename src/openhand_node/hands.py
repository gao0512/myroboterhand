			...
			for i in range(num_servos):
				...
				if series == "RX":
					if (self.motorDir[i] ==1 and enc > 512) or (self.motorDir[i] == -1 and enc < 512):
						...
						user_input = input("Your encoder position for motor index " + str(i) + " may cause the motor to move backwards and break gears. We recommend you resetting the fingers to prevent gear shear, proceed? [ENTER]")
					else:
				...
				#These would then be the MX and XM motors
					...
					print( "Failsafe is incorporated to prevent gear shear in Model O")
					print( "Motor encoder postion: ", enc)
					user_input = input("As an XM Motor, we can automatically fix this issue for motor " + str(i) + ", proceed? [ENTER]")
...
class Model_O(OpenHand):
	...
	max_close = 0.6 #max motor movement from open to close for individual hand
	...
	def pinch_close(self,amnt=0.45):
	...
	def pinch_object_move(self,delta_amnt=0.03, left=False, down = True): #These cannot be the same or else we will move diagonal
		adduct_loc, enc = self.readHand()
		if(adduct_loc[0] > 0.05):
